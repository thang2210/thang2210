...
int main()
{
  ...
  cin >> n >> m;
  int a[n], b[m];
  int min_vl = 1e9, max_vl = -1e9;
  for (int i = 0; i < n; i++)
  {
    cin >> a[i];
    cnt[a[i]]++;
    min_vl = min(min_vl, a[i]);
    max_vl = max(max_vl, a[i]);
  }
  for (int i = 0; i < m; i++)
  {
    cin >> b[i];
    cnt[b[i]]++;
    min_vl = min(min_vl, b[i]);
    max_vl = max(max_vl, b[i]);
  }
  for (int i = min_vl; i <= max_vl; i++)
  {
    if (cnt[i])
      cout << i << " ";
  }
...
// bai tiep Bài 26. Trộn 2 dãy và sắp xếp
...
int main()
{
  int n;
  cin >> n;
  int a[10] = {1000, 500, 200, 100, 50, 20, 10, 2, 1};
  int count = 0;
  for (int i = 0; i < 10; i++)
  {
    count += n / a[i];
