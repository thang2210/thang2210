...
int main(){
    int n;
    cin >> n;
    int a[n]; // khoi toa mang voi n phan tu 
    // khoi toa vong lap for each
    for ( auto &x : a) cin >> x;
    int q; cin >> q; // khai bao truy van 
    while (q--){
        int l,r; // khoi tao daon can truy van l = left , r = right
        cin >> l >> r;
        // khoi tao vong lap for cho cahy tu l den r
        int sum =0;
        for (int i = l ; i <= r ;i++){
            sum += a[i];
        }
