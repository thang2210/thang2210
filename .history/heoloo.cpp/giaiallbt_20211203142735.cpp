...
// tinh frefix sum cua mang 1 chieu va 2 chieu 
// mang 1 chieu cach 1 : 
/*#include<iostream>
using namespace std;
int main(){
...
// truy van mang 2 chieu 
// truy van trong doan 2 * 2
/*#include<iostream>
using namespace std;
int main(){
    int n,m;
...
int main(){
    ...
    for (int i =0 ;i < n ;i++){
        for (int j =0 ;j < m ;j++){
            fre[i][j] = fre[i-1][j] + fre[i][j-1] - fre[i-1][j-1] + a[i][j];
        }
    }
    int q;
    cin >> q;
    while (q){
        int h1 ,h2 ,c1 ,c2;
        cin >> h1 >> h2 >> c1 >> c2;
        cout << (fre[h2][c2] - fre[h1-1][c2] - fre[h2][c1-1] + fre[h1-1][c1-1]);
    }
    return 0;
}